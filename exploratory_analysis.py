...
c) Examine if there are values greater than 3+st from the mean or lower than -3std
'''
for col in df.columns[1:]: #no user_id's column
    print('_'*100)
    print('----- +-3 Standard Deviations, Quantiles, IQR, Lower and Upper Limits ------')
    ...
    Are there values higher or lower than +3 stdeviations 
                                       or -3 stdeviations from the mean ?
    '''
    '''
    +3 std from the mean
    '''
    ...
    if df[col].max() > df[col].mean() + (df[col].std() *3):
        ...
        print(f"Column {df[col].name} does not include outlier points greater than +3 std.")
        '''
        -3std from the mean
        '''
    print("Values lower than -3 std from the mean :")
    if df[col].min() < df[col].mean() + (df[col].std() *3):
        print(f"In column {df[col].name} there are values lower than -3 stdeviations from the mean.")
    else:
        print(f"Column {df[col].name} does not include outlier points lower than -3std.")
        '''
        Interquartile Range : upper_limit = Q3 + 1.5 * IQR
                              lower_limit = Q3 - 1.5 * IQR
        '''             
    print('Quartiles-IQR-Upper Limit, Lower Limit')
    ...
    print(f"Upper limit for {df[col].name} column is {upper_limit}.")
    lower_limit = df[col].quantile(0.75) - 1.5 * (df[col].quantile(0.75) - df[col].quantile(0.25))
    print(f"Upper limit for {df[col].name} column is {lower_limit}.") 
