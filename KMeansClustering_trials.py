...
'''
Clustering with k : 3 , Robust Scaling
'''

np.random.seed(40)
kmeans_3clusters = KMeans(n_clusters = 3, init = 'k-means++', random_state = 42)

kmeans_3clusters.fit(X_robust_features)

#Predictions
predictions_3clusters = kmeans_3clusters.predict(X_robust_features)
centroids_3clusters = kmeans_3clusters.cluster_centers_
print(f"For k = 3, number of data points in each cluster:\n{pd.DataFrame(predictions_3clusters).value_counts()}")


'''
Clustering with k = 5, Robust Scaling
'''
np.random.seed(42)
...
print(f"For k : 3 mean square distance between each instance and its closest centroid is {kmeans_3clusters.inertia_}")
...
print("-" *100)
print(f"For k : 3 between  clusters'  distance is {silhouette_score(X_robust_features, predictions_3clusters )}")
print(f"For k : 4 between  clusters'  distance is {silhouette_score(X_StandardScaled_feats, predictions_4clusters )}")
print(f"For k : 5 between  clusters'  distance is {silhouette_score(X_robust_features, predictions_5clusters )}")
print(f"For k : 6 between  clusters'  distance is {silhouette_score(X_StandardScaled_feats, predictions_6clusters )}")
...
X_features = df.iloc[:, 1:].values #array of features without scaling
X_features
...
np.random.seed(40)
kmout_3clusters_no_scaled = KMeans(n_clusters = 3, init = 'k-means++', random_state = 42)

kmout_3clusters_no_scaled.fit(X_features)

#Predictions
predictions_3clusters_no_scaled = kmout_3clusters_no_scaled.predict(X_features)
centroids_3clusters_no_scaled = kmout_3clusters_no_scaled.cluster_centers_
print(f"For k = 3, number of data points in each cluster WITHOUT SCALING :\n{pd.DataFrame(predictions_3clusters_no_scaled).value_counts()}")
print(f"For k : 3 mean square distance between each instance and its closest centroid WITHOUT SCALING is {round(kmout_3clusters_no_scaled.inertia_, 2)}")
print(f"For k : 3 between  clusters'  distance WITHOUT SCALING is {round(silhouette_score(X_features, predictions_3clusters_no_scaled), 4)}")
...
np.random.seed(40)
kmout_4clusters_no_scaled = KMeans(n_clusters = 4, init = 'k-means++', random_state = 42)

kmout_4clusters_no_scaled.fit(X_features)

#Predictions
predictions_4clusters_no_scaled = kmout_4clusters_no_scaled.predict(X_features)
centroids_4clusters_no_scaled = kmout_4clusters_no_scaled.cluster_centers_
print(f"For k = 4, number of data points in each cluster WITHOUT SCALING :\n{pd.DataFrame(predictions_4clusters_no_scaled).value_counts()}")
print(f"For k : 4, mean square distance between each instance and its closest centroid WITHOUT SCALING is {round(kmout_4clusters_no_scaled.inertia_, 2)}")
print(f"For k : 4, between  clusters'  distance WITHOUT SCALING is {round(silhouette_score(X_features, predictions_4clusters_no_scaled), 4)}")
...
np.random.seed(40)
kmout_5clusters_no_scaled = KMeans(n_clusters = 5, init = 'k-means++', random_state = 42)

kmout_5clusters_no_scaled.fit(X_features)

#Predictions
predictions_5clusters_no_scaled = kmout_5clusters_no_scaled.predict(X_features)
centroids_5clusters_no_scaled = kmout_5clusters_no_scaled.cluster_centers_
print(f"For k = 5, number of data points in each cluster WITHOUT SCALING :\n{pd.DataFrame(predictions_5clusters_no_scaled).value_counts()}")
print(f"For k : 5, mean square distance between each instance and its closest centroid WITHOUT SCALING is {round(kmout_5clusters_no_scaled.inertia_, 2)}")
print(f"For k : 5, between  clusters'  distance  WITHOUT SCALING is {round(silhouette_score(X_features, predictions_5clusters_no_scaled), 4)}")
...
np.random.seed(40)
kmout_6clusters_no_scaled = KMeans(n_clusters = 6, init = 'k-means++', random_state = 42)

kmout_6clusters_no_scaled.fit(X_features)

#Predictions
predictions_6clusters_no_scaled = kmout_6clusters_no_scaled.predict(X_features)
centroids_6clusters_no_scaled = kmout_6clusters_no_scaled.cluster_centers_
print(f"For k = 6, number of data points in each cluster WITHOUT SCALING :\n{pd.DataFrame(predictions_6clusters_no_scaled).value_counts()}")
print(f"For k : 6, mean square distance between each instance and its closest centroid WITHOUT SCALING is {round(kmout_6clusters_no_scaled.inertia_, 2)}")
print(f"For k : 6, between  clusters'  distance  WITHOUT SCALING is {round(silhouette_score(X_features, predictions_6clusters_no_scaled), 4)}")

#5 - Means Clustering results good performance on Within Sum Of Squares 
#3 - Means Clustering results good performance on Between clusters' distance